 ...
 * @author Oussama
 */
class MainSimplePanel extends JPanel {
    ...
    public MainSimplePanel(String fileName) throws IOException {
        ...
        _tooltip = new CustomTipTool[nbrButton];

        for (i = 3; i < nbrButton; i++) {
            ...
            _tooltip[i] = new CustomTipTool();
            _tooltip[i].setComponent(buttons[i]);
            buttons[i].setToolTipText(hint[i]);
            this.add(buttons[i]);
        }
        buttons[5].setBounds(748, 430, 90, 90);
        buttons[4].setBounds(610, 430, 90, 90);
        buttons[3].setBounds(477, 430, 90, 90);
